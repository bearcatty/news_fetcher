...
DEFAULT_TIMEOUT = float(os.getenv("LM_STUDIO_TIMEOUT", "300"))  # 增加到300秒
CONNECT_TIMEOUT = 30.0  # 连接超时
READ_TIMEOUT = 300.0  # 读取超时
...
class LMStudioClient:
    ...
    async def chat_completion(
        ...
        try:
            ...
            logger.debug(f"Response size: {len(str(result))} bytes")
            return result
        except httpx.TimeoutException as e:
            logger.error(f"Timeout calling LM Studio (timeout={DEFAULT_TIMEOUT}s): {e}")
            raise
        except httpx.HTTPError as e:
            logger.error(f"HTTP error calling LM Studio: {e}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Error calling LM Studio: {e}", exc_info=True)
            raise
